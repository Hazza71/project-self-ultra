/**
 * Synchronous SHA-256 (UTF-8) for stable catalog IDs.
 * Must produce the same digest in Node tests and the Expo runtime.
 */
...
function utf8Bytes(message: string): Uint8Array {
  ...
  const escaped = unescape(encodeURIComponent(message));
...
export function sha256Hex(message: string): string {
  ...
  const padLen = (data.length + 9) % 64 === 0 ? 9 : 64 - ((data.length + 9) % 64) + 9;
